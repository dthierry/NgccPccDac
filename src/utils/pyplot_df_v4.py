...
def main():
    ...
    ax.bar(h, remain_frs, label="Inv Fresh")
    ax.bar(h, c, bottom = remain_frs, label="vAbs")
    ax.bar(h, d, bottom = remain_frs + c, label="1-h Abs")
    ax.bar(h, remain_sat, bottom = remain_frs + c + d, label="Inv Sat")
    ...
    #df = pd.read_csv("../mods/df_pow.csv")
    fig, ax = plt.subplots()
    ax.stackplot(df.index, df["PowGasTur"],
            df["PowHp"],
            df["PowIp"],
            df["PowLp"],
    ...
    ax.set_ylabel("Power (MWh)")
    ax.legend()
    fig.savefig("Powah_s_stacked.png")
    fig.clf()
    ...
    fig, ax = plt.subplots()
    ax.stackplot(trd.index, trd["Co2CapPcc"], trd["Co2CapDacFlue"], trd["NotCaptCo2"], trd["Co2CapDacAir"],
            labels=["Co2CapPcc", "Co2CapDacFlue", "NotCaptCo2", "Co2CapDacAir"],
            colors=["lightgray", "skyblue", "lightcoral", "lavender"])
    ax.set_title("CO2 Capture")
    ax.set_xlabel("Hour")
    ax.set_ylabel("Tonne CO2")
    ax.legend()
    fig.savefig("co2capture.png")
    fig.clf()
    ...
    # Co2 dac only
    fig, ax = plt.subplots()
    ax.stackplot(trd.index, trd["Co2CapDacFlue"], trd["NotCaptCo2"], trd["Co2CapDacAir"],
    ...
    w = 0.35
    fig, ax = plt.subplots()
    ax.bar(x - w/2, df["SteaUsePcc"], w, bottom=0, label="used PCC")
    ax.bar(x - w/2, df["PccSteaSlack"], w, bottom=df["SteaUsePcc"], label="available PCC")

    ax.bar(x + w/2, df["DacTotal"], w, bottom=0, label="used DAC")
    ax.bar(x + w/2, df["DacSteaSlack"], w, bottom=df["DacTotal"], label="available DAC")
    ...
    fig, ax = plt.subplots()
    ax.stackplot(h, df["DacSteaBaseDuty"], df["SideSteaDac"], labels=["Base", "Add-Allocated"])
    ax.legend()
    ax.set_xlabel("Hour")
    ax.set_ylabel("Heat (MMBTU)")
    ax.set_title("Dac Steam")
    plt.savefig("dac_Stea.png")
    plt.close()
