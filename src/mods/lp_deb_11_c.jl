...
aAuxGt = Dict(0 => 1, 1 => aAuxRateGas * slopeFactor, 2 => aAuxRateGas)
aPowHp = Dict(0 => 1, 1 => aPowHpEload * slopeFactor, 2 => aPowHpEload)
aPowIp = Dict(0 => 1, 1 => aPowIpEload * slopeFactor, 2 => aPowIpEload)
aCcReb = Dict(0 => 1, 1 => aCcRebDutyEload * slopeFactor, 2 => aCcRebDutyEload)
...
## b
bPowGt = Dict(0 => 0, 
    1 => 60 * (-aPowGt[1] + aPowGt[2]) + bPowHpEload, 
    2 => bPowGasTeLoad)
...
# Gas Turbine
@constraint(m, powGasTur[i = 0:tHorz], 
            xPowGasTur[i] == aPowGasTeLoad * xeLoad[i] 
            + bPowGasTeLoad
           )
# 
@constraint(m, fuelEq[i = 0:tHorz], 
            xFuel[i] == aFuelEload * xeLoad[i] 
            + bFuelEload
           )
# 
@constraint(m, co2FuelEq[i = 0:tHorz], 
            xCo2Fuel[i] == aEmissFactEload * xeLoad[i] 
            + bEmissFactEload
           )

@constraint(m, auxPowGasT[i = 0:tHorz],
            xAuxPowGasT[i] == aAuxRateGas * xeLoad[i] 
            + bAuxRateGas
           )
# 
# Steam
...
@constraint(m, powHpEq[i = 0:tHorz], 
            xPowHp[i] == aPowHpEload * xeLoad[i] 
            + bPowHpEload
           )
# 
@constraint(m, powIpEq[i = 0:tHorz], 
            xPowIp[i] == aPowIpEload * xeLoad[i] 
            + bPowIpEload
           )
...
@constraint(m, ccRebDutyEq[i = 0:tHorz],
            xCcRebDuty[i] == 
            aCcRebDutyEload * xeLoad[i] 
            + bCcRebDutyEload
           )

@constraint(m, dacSteaDutyEq[i = 0:tHorz],
            xDacSteaBaseDuty[i] == aDacSteaBaseEload * xeLoad[i] 
            + bDacSteaBaseEload
           )


@constraint(m, sideSteaEloadEq[i = 0:tHorz],
            xAllocSteam[i] == aSideSteaEload * xeLoad[i] 
            + bSideSteaEload
           )
...
@constraint(m, auxPowSteaTEq[i = 0:tHorz],
            xAuxPowSteaT[i] == aAuxRateStea * xeLoad[i] 
            + bAuxRateStea 
           )
