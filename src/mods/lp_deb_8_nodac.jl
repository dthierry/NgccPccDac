...
aSorbAmountFreshFlue = 0.0   # Tonne sorb (Max. heat basis)
aSorbAmountFreshAir = 0.0  # Tonne sorb (Max. heat basis)
...
@constraint(m, 
  storeFairEq[i = 0:tHorz - 1], 
  xFair[i + 1] == xFair[i] - vAbsAir[i] + xR1Air[i])
@constraint(m, 
  storeSaireq[i = 0:tHorz - 1], 
...
@constraint(m, endDacAirEq, xFair[tHorz] == aSorbAmountFreshAir)
@constraint(m, endA1AirEq, xA1Air[tHorz] == 0.)
@constraint(m, endA2AirEq, xA2Air[tHorz] == 0.)
@constraint(m, endAR1AirEq, xR1Air[tHorz] == 0.)
@constraint(m, endSsAirEq, xSair[tHorz] == 0.)
#
...
df_dac_flue = DataFrame(
                     ...
                     :xR1Flue => Float64[])
for i in 0:tHorz - 1
    push!(df_dac_flue,(float(i), value(xFflue[i]), value(xSflue[i]),
