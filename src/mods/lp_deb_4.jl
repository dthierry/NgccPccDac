...
@variable(m, 0 <= a1[0:tHorz]) # State
@variable(m, 0 <= a2[0:tHorz]) # State

@variable(m, 0 <= aR0[0:tHorz - 1])
@variable(m, 0 <= aR1[0:tHorz])  # State
...
@constraint(m, aRdEq[i = 0:tHorz - 1], aR1[i + 1] == aR0[i])
...
@constraint(m, icA2Eq, a2[0] == 0.)
@constraint(m, icAR1Eq, aR1[0] == 0.)
...
# 24
@constraint(m, co2CapDacEq[i = 0:tHorz - 1], eCo2CapDac[i] == pSorbCo2Cap * aR1[i])
# 25
...
for i in 0:tHorz - 1
    push!(df_dac,(value(sF[i]), value(sS[i]),
                  ...
                  value(a0[i]), value(a1[i]), value(a2[i]),
                  value(aR0[i]), value(aR1[i])))
end
