...
hSlice = 2  # the number of slices of a given hour
yGas = 0.7
...
vCo2PccVent = Dict((i, j) => 1 for i in 0:tHorz for j in 1:hSlice)
...
xA0Flue = Dict((i, j) => 1 for i in 0:tHorz for j in 0:hSlice)
...
xR0Flue = Dict((i, j) => 1 for i in 0:tHorz for j in 0:hSlice)
...
xA0Air = Dict((i, j) => 1 for i in 0:tHorz for j in 0:hSlice)
xR0Air = Dict((i, j) => 1 for i in 0:tHorz for j in 0:hSlice)
...
@constraint(m, powHpEq[i = 0:tHorz, j = 1:hSlice], 
            xPowHp[i, j] == aPowHpEload * yGas 
            + bPowHpEload
           )
# 
@constraint(m, powIpEq[i = 0:tHorz, j = 1:hSlice], 
            xPowIp[i, j] == aPowIpEload * yGas 
            + bPowIpEload
           )
...
#            xA0Flue[i, j] == xA0Flue[i, j]
#           )
#@constraint(m, xR0FlueEq[i = 0:tHorz, j=0:hSlice], 
#            xR0Flue[i, j] == xR0Flue[i, j]
#           )
#
#: "State equation"
...
@constraint(m, storeSflueeq[i = 0:tHorz, j = 1:hSlice], 
            xSflue[i, j] == xSflue[i, j-1] - xR0Flue[i, j-1] + xA2Flue[i, j-1]
           )
# Initial conditions
...
#            xA0Air[i, j] == xA0Air[i, j]
#           )
#@constraint(m, xR0AirEq[i = 0:tHorz, j = 0:hSlice], 
#            xR0Air[i, j] == xR0Air[i, j]
#           )
# "State equation"
