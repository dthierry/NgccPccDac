...
@constraint(m, dacSteaDutyEq[i = 0:tHorz - 1],
            xDacSteaBaseDuty[i] == aDacSteaBaseEload * yGasTelecLoad[i] 
            + bDacSteaBaseEload
           )
...
@constraint(m, storeFflueeq[i = 0:tHorz - 1], 
            xFflue[i + 1] == xFflue[i] - vAbsFlue[i] + xR1Flue[i])
@constraint(m, storeSflueeq[i = 0:tHorz - 1], 
            xSflue[i + 1] == xSflue[i] - vRegFlue[i] + xA2Flue[i])
# Initial conditions
...
#
@constraint(m, co2StorDacFlueEq[i = 0:tHorz - 1], 
            xCo2StorDacFlue[i] == aSorbCo2CapFlue * xSflue[i])
#
@constraint(m, co2CapDacFlueEq[i = 0:tHorz - 1], 
            xCo2CapDacFlue[i] == aSorbCo2CapFlue * xR1Flue[i])
#
@constraint(m, steamUseDacFlueEq[i = 0:tHorz - 1], 
            xSteaUseDacFlue[i] == aSteaUseRateDacFlue * xCo2CapDacFlue[i])
#
@constraint(m, powUseDacFlueEq[i = 0:tHorz - 1], 
            xPowUseDacFlue[i] == aPowUseRateDacFlue * xCo2CapDacFlue[i])
# Equal to the amount vented, at least in flue mode.
@constraint(m, co2DacFlueVentEq[i = 0:tHorz - 1], 
            xCo2DacVentFlue[i] == xCo2DacFlueIn[i] - xCo2CapDacFlue[i])
...
@constraint(m, storeFairEq[i = 0:tHorz - 1], 
            xFair[i + 1] == xFair[i] - vAbsAir[i] + xR1Air[i])
@constraint(m, storeSaireq[i = 0:tHorz - 1], 
            xSair[i + 1] == xSair[i] - vRegAir[i] + xA2Air[i])
...
@constraint(m, steamUseDacAirEq[i = 0:tHorz - 1], 
            xSteaUseDacAir[i] == aSteaUseRateDacAir * xCo2CapDacAir[i])
# 
@constraint(m, powUseDacAirEq[i = 0:tHorz - 1], 
            xPowUseDacAir[i] == aPowUseRateDacAir * xCo2CapDacAir[i])
...
@constraint(m, co2dacnomyr,
            xCo2DacNomYr == (xSorbFreshAir * aSorbCo2CapAir * 8760/3
          + xSorbFreshFlue * aSorbCo2CapFlue * 8760/3)/10  # assume this is for 10 years :S
           )
...
# Co2 Compression
# 
@constraint(m, co2CompEq[i = 0:tHorz - 1], 
            xCo2Comp[i] == xCo2CapPcc[i] 
            + xCo2CapDacFlue[i] + xCo2CapDacAir[i])
# 
@constraint(m, powUseCompEq[i = 0:tHorz - 1], 
