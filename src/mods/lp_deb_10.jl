...
@variable(m, xAuxPow[0:tHorz, 1:hSlice])
...
@constraint(m, auxPowGasT[i = 1:tHorz, j = 1:hSlice],
...
@constraint(m, pwleq[i = 0:tHorz, j = 2:tHorz],
            yGasTelecLoad[i, 1] == yGasTelecLoad[i, j]
           )
@constraint(m, pwssd[i = 0:tHorz, j = 2:tHorz],
           xSideSteaDac[i, 1] == xSideSteaDac[i, j]  
           )

# Continuity of states
@constraint(m, cxfflue[i = 1:tHorz], xFflue[i, 0] == xFflue[i - 1, hSlice])
@constraint(m, ca1flue[i = 1:tHorz], xA1Flue[i, 0] == xA1Flue[i - 1, hSlice])
@constraint(m, ca2flue[i = 1:tHorz], xA2Flue[i, 0] == xA2Flue[i - 1, hSlice])
@constraint(m, cxsflue[i = 1:tHorz], xSflue[i, 0] == xSflue[i - 1, hSlice])

@constraint(m, cxfair[i = 1:tHorz], xFair[i, 0] == xFair[i - 1, hSlice])
@constraint(m, ca1air[i = 1:tHorz], xA1Air[i, 0] == xA1Air[i - 1, hSlice])
@constraint(m, ca2air[i = 1:tHorz], xA2Air[i, 0] == xA2Air[i - 1, hSlice])
@constraint(m, cxsair[i = 1:tHorz], xSair[i, 0] == xSair[i - 1, hSlice])
